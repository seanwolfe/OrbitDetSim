...
def run_IOD_hyperparameter_run_par_resumable_bhcolloc(config):
    # --- MPI setup ---
    comm = MPI.COMM_WORLD
    rank = comm.Get_rank()
    size = comm.Get_size()
    ...
    # --- Hyperparam grids (don't materialize huge lists more than needed) ---
    p1 = config['range_step']
    p2 = config['range_rate_step']
    p3 = config.get('temperature', [])
    p4 = config['colloc_ratio']
    p5 = config['delta_tf']
    optimizer = config['optimizer']

    if optimizer == 'CONSTRAINED_BASIN_HOPPING':
        combo_iter = itertools.product(p1, p2, p3, p4, p5)
    else:
        combo_iter = itertools.product(p1, p2)
    ...
    def key_of(row):
        """Return the canonical numeric key tuple for a row or CSV dict row."""
        return (
            float(row.get("DELTA_RHO", 0) or 0),
            float(row.get("DELTA_RHO_DOT", 0) or 0),
            float(row.get("TEMPERATURE", 0) or 0),
            float(row.get("LAYER_RATIOS", 0) or 0),
            float(row.get("TIME_DELTA_DAYS", 0) or 0),
            int(row.get("RUN_NUMBER", 0) or 0),
        )
...
def run_IOD_hyperparameter_run_par_resumable_ns(config):
    # --- MPI setup ---
    comm = MPI.COMM_WORLD
    rank = comm.Get_rank()
    size = comm.Get_size()
    ...
    def key_of(row):
        """Return the canonical numeric key tuple for a row or CSV dict row."""
        return (
            int(row.get("HIDDEN_DIMENSION", 0) or 0),
            int(row.get("NUMBER_OF_ITERATIONS", 0) or 0),
            int(row.get("RUN_NUMBER", 0) or 0),
        )
    ...
    for (combo, run_idx) in task_stream():
        ...
        if dynamics == 'NBD' and observer == 'SPACE' and optimizer == 'CONSTRAINED_BASIN_HOPPING':
            ...
            parameters = {
                ...
                'TOTAL_POINTS': combo[0],
                'SAMPLING_METHOD': "uniform",
                'LAYER_RATIOS': [(0., m_12), (m_12, m_12 + m_2), (m_12 + m_2, 1.)],
                'INPUT_RANGE': (-1, 1),
                'HIDDEN_DIMENSION': combo[0],
                'PHYSICS_WEIGHT': 1e3,
                'LAMBDA_DIST': 1e-2,
                'WEIGHT_SCALE_FACTOR': 1e-2,
                'NUMBER_OF_ITERATIONS': combo[1],
                'TEMPERATURE': 1e8,
                'X_TOLERANCE': 1e-15, 'F_TOLERANCE': 1e-15,
                'MAX_FUNCTION_EVAL': 1000, 'MAX_ITERATiONS': 1000,
                'G_TOLERANCE': 1e-15, 'RUN_NUMBER': run_idx,
                'MIN_RHO': 0.0006684587122, 'MAX_RHO': 0.06684587122,
                'MIN_RHO_DOT': -0.05033557046, 'MAX_RHO_DOT': 0.05033557046,
                'DELTA_RHO': 6.68459e-8, 'DELTA_RHO_DOT': 0.0016722408
            }
            if viz_flag:
                config['lambda'] = parameters['TIME_DELTA']
                config['run_idx'] = run_idx
        ...
        # ---- spacecraft state along the same epochs grid ----
        # ensure sc_positions/sc_velocities have same length as epochs
        sc_positions, sc_velocities = util.interpolate_sc_traj(
            data[1], data[-1], data[2], num_points=len(epochs)
        )

        # ---- topocentric RMSEs (RA/Dec/Range and rates) ---
        # If states might be (3,N), convert to (N,3)
        final_pos_np = rmse_df[["IOD_X","IOD_Y","IOD_Z"]].values
        final_vel_np = rmse_df[["IOD_VX","IOD_VY","IOD_VZ"]].values
        true_pos_np = rmse_df[["TRUE_X","TRUE_Y","TRUE_Z"]].values
        true_vel_np = rmse_df[["TRUE_VX","TRUE_VY","TRUE_VZ"]].values

        topo_errs = util.topocentric_rmse(
            final_pos_np, final_vel_np,
            true_pos_np, true_vel_np,
        ...
        def build_extras(pos_rmse, vel_rmse, topo_errs, comp_time, optimal_bh, file_used, file_name):
            return {
                "POS_RMSE": float(pos_rmse),
                "VEL_RMSE": float(vel_rmse),
                "RA_RMSE_RAD": float(topo_errs["RA_RMSE_RAD"]),
                "DEC_RMSE_RAD": float(topo_errs["DEC_RMSE_RAD"]),
                "RHO_RMSE": float(topo_errs["RHO_RMSE"]),
                "RA_DOT_RMSE_RADPS": float(topo_errs["RA_DOT_RMSE_RADPS"]),
                "DEC_DOT_RMSE_RADPS": float(topo_errs["DEC_DOT_RMSE_RADPS"]),
                "RHO_DOT_RMSE": float(topo_errs["RHO_DOT_RMSE"]),
                "COMPUTATION_TIME_SEC": float(comp_time),
                "OPTIMAL_BH_ITERATION": float(optimal_bh),
                "FILE_USED": str(file_used),
                "SAVED_AS": str(file_name),
        ...
        extras = build_extras(pos_rmse, vel_rmse, topo_errs, comp_time, optimal_bh, file_used, file_name)
        row = params_to_row(parameters, run_idx, extras)
...
run_IOD_hyperparameter_run_par_resumable_ns(config)
