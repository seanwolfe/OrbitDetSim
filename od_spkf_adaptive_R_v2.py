...
def od_setup_from_iod(config, row, *, util, sp, eps=1e-12):
    ...
    sc_eme_ae_kms = util.geo_eclip_to_geo_eme_generic(sc_geoeclip_ae_kms, layout="(batch, 6)")
    ...
    *,
    ...
    for j in range(N):
        ...
        for sid in detecting_ids:
            ...
            # Nominal sensor covariance from config values:
            #   sigma_ra, sigma_dec, sigma_pointing.
            # This also acts as the PSD floor for adaptive R.
            R_sensor = ukf.build_R(
                ra_meas,
                dec_meas,
                sigma_ra=ukf.sigma_ra,
                sigma_dec=ukf.sigma_dec,
                sigma_pointing=ukf.sigma_pointing,
                sigma_meas=None,
            )

            # Inflated single-observer covariance from config value:
            #   sigma_inflated, used isotropically for RA and Dec.
            R_inflated = ukf.build_R(
                ra_meas,
                dec_meas,
                sigma_meas=ukf.sigma_inflated,
            )

            update_info = ukf.update_angles_unitvec(
                z_hat,
                r_obs_km,
                R_sensor,
                R_inflated=R_inflated,
            ...
            epoch_updates.append({
                ...
                "R_mode": update_info.get("R_mode"),
                "R_lambda": update_info.get("R_lambda"),
                "R_eff": update_info.get("R_eff"),
                "R_sensor": R_sensor.copy(),
                "R_inflated": R_inflated.copy(),
                "innovation": update_info.get("innovation"),
                "S_model": update_info.get("S_model"),
            })
...
class OD_UKF:
    ...
    def __init__(
        ...
        *,
        ...
        sigma_meas=None,                   # optional isotropic angular noise override in meas_units
        sigma_inflated=None,               # isotropic RA/Dec uncertainty for single-observer mode
        adaptive_R_window=20,              # innovation-history window for adaptive R
        adaptive_R_min_samples=3,          # minimum samples before adaptive R is trusted
        adaptive_R_tau=3.0,                # exponential decay constant for inflated->adaptive transition
        use_adaptive_R=True,
        ukf_alpha=1e-3,
        ukf_beta=2.0,
        ukf_kappa=0.0,
        ...
        # Adaptive/inflated R handling. If sigma_inflated is omitted, fall back
        # to sigma_meas if supplied, otherwise the larger RA/Dec uncertainty.
        if sigma_inflated is None:
            if self.sigma_meas is not None:
                sigma_inflated = self.sigma_meas
            else:
                sigma_inflated = max(float(sigma_ra), float(sigma_dec))
        self.sigma_inflated = float(sigma_inflated)
        self.adaptive_R_window = int(adaptive_R_window)
        self.adaptive_R_min_samples = int(adaptive_R_min_samples)
        self.adaptive_R_tau = float(adaptive_R_tau)
        self.use_adaptive_R = bool(use_adaptive_R)
        self.innov_hist = []
        self.multi_observer_update_count = 0
        self._was_multi_observer = False
        self.last_R_eff = None
        self.last_R_mode = None
        self.last_R_lambda = None
    ...
    def update_angles_unitvec(
        ...
        *,
        ...
        """
        UKF update for a unit-LOS measurement with observability-aware R handling.

        R selection:
          - n_observers < 2:
                use R_inflated, built from sigma_inflated in the calling code.
          - n_observers >= 2:
                use UKF adaptive R = sample innovation covariance - S_model,
                PSD-floored so R_adaptive >= R_hat in the Loewner sense.
                During the transition from single to multi-observer mode, blend:
                    R_eff = lambda * R_inflated + (1-lambda) * R_adaptive
                with lambda = exp(-m / adaptive_R_tau), m starting at 1.

        R_hat is the nominal sensor covariance in unit-vector space and is also
        the physical lower floor for adaptive R.
        """
        z = np.asarray(z_rhohat, dtype=float).reshape(3)
        r_obs = np.asarray(r_obs_km, dtype=float).reshape(3)
        R_sensor = self._symmetrize(np.asarray(R_hat, dtype=float).reshape(3, 3))

        if R_inflated is None:
            R_inflated = R_sensor
        R_inflated = self._symmetrize(np.asarray(R_inflated, dtype=float).reshape(3, 3))

        if use_adaptive_R is None:
            use_adaptive_R = self.use_adaptive_R
        ...
        # UKF predicted measurement covariance WITHOUT measurement noise.
        ...
        # Select effective measurement covariance.
        if int(n_observers) < 2:
            R_eff = R_inflated.copy()
            R_mode = "single_observer_inflated"
            lam = 1.0
            self._was_multi_observer = False
        else:
            if not self._was_multi_observer:
                self.multi_observer_update_count = 0
                self._was_multi_observer = True

            if use_adaptive_R:
                R_adaptive = self.estimate_adaptive_R(
                    innov=innov,
                    S_model=S_model,
                    R_floor=R_sensor,
                )
            else:
                R_adaptive = R_sensor.copy()

            # First multi-observer update uses m=1, not m=0, so it is not
            # purely inflated. Subsequent multi-observer updates decay further.
            m = self.multi_observer_update_count + 1
            tau = max(self.adaptive_R_tau, self.eps)
            lam = float(np.exp(-float(m) / tau))
            R_eff = lam * R_inflated + (1.0 - lam) * R_adaptive
            R_eff = self._symmetrize(R_eff)
            R_mode = "multi_observer_adaptive_blend" if use_adaptive_R else "multi_observer_sensor_blend"
            self.multi_observer_update_count += 1
        ...
        self.last_R_mode = R_mode
        self.last_R_lambda = lam
    ...
    def estimate_adaptive_R(self, innov, S_model, R_floor):
        """
        UKF-compatible adaptive measurement covariance.

        Uses:
            Cov(innovation) = S_model + R

        Therefore:
            R_adaptive = Cov_sample(innovation) - S_model

        The result is PSD-floored so that:
            R_adaptive >= R_floor

        This is a Loewner/PSD matrix floor, not elementwise clipping.
        """
        innov = np.asarray(innov, dtype=float).reshape(3)
        S_model = self._symmetrize(np.asarray(S_model, dtype=float).reshape(3, 3))
        R_floor = self._symmetrize(np.asarray(R_floor, dtype=float).reshape(3, 3))

        self.innov_hist.append(innov.copy())
        if len(self.innov_hist) > self.adaptive_R_window:
            self.innov_hist.pop(0)

        if len(self.innov_hist) < self.adaptive_R_min_samples:
            return R_floor.copy()

        V = np.asarray(self.innov_hist, dtype=float)
        Sigma_innov = np.cov(V.T, bias=True)
        Sigma_innov = self._symmetrize(np.asarray(Sigma_innov, dtype=float).reshape(3, 3))

        R_adaptive = self._symmetrize(Sigma_innov - S_model)
        return self.floor_matrix_psd(R_adaptive, R_floor)

    def floor_matrix_psd(self, R_candidate, R_floor, eps=None):
        """
        Enforce R_candidate >= R_floor in PSD/Loewner order.

        This is covariance flooring in matrix space:
            R_out = R_floor + PSD_part(R_candidate - R_floor)
        """
        if eps is None:
            eps = self.eps

        R_candidate = self._symmetrize(np.asarray(R_candidate, dtype=float).reshape(3, 3))
        R_floor = self._symmetrize(np.asarray(R_floor, dtype=float).reshape(3, 3))

        A = self._symmetrize(R_candidate - R_floor)
        eigval, eigvec = np.linalg.eigh(A)
        eigval = np.maximum(eigval, float(eps))
        R_out = R_floor + eigvec @ np.diag(eigval) @ eigvec.T
        return self._symmetrize(R_out)
