...
def generate_data(config, parameters):
    ...
    sin_ra_meas, cos_ra_meas, sin_dec_meas, cos_dec_mean = torch.sin(ra), torch.cos(ra), torch.sin(dec), torch.cos(dec)

    return ([sin_ra_meas, cos_ra_meas, sin_dec_meas, cos_dec_mean], torch.tensor(observer_positions, dtype=torch.float32),
...
def solve(epochs_nd_norm_reshaped_tensor, y_obs, obs_indices, observer_positions, colloc_epochs, c, configuration,
    ...
    beta_tensor = np.asarray(res.x).reshape(q, H_size)
    Y_pred = H_matrix @ beta_tensor.T  # (N, q)
    Y_dot_pred = c * H_dot @ beta_tensor.T
