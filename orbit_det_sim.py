...
def run_sim_runnumbers_MPI_getIOD_data_bychunk_helio(config):
    ...
    for i, file_i in enumerate(all_files):
        ...
        for jdx, detected_minimoon in detected_appended_pop_chunk.iterrows():
            ...
            asteroid_state = util.helio_eclip_to_geo_secr_generic(asteroid_states_aud,
                                                                                 earthasteroid_states_aud, layout="time")
            ...
            spacecraft_state = util.helio_eclip_to_geo_secr_generic(integrated_states_aud,
                                                                                   earth_states_aud, layout="time")

            # convert back to helio based on asteroid epoch
            new_spacecraft_state_helio = util.geo_secr_to_helio_eclip_generic(spacecraft_state,
                                                                                             earthasteroid_states_aud,
                                                                              layout="time")
            new_asteroid_state_helio = util.geo_secr_to_helio_eclip_generic(asteroid_state,
                                                                                           earthasteroid_states_aud,
                                                                            layout="time")
...
def run_sim_runnumbers_MPI_getIOD_data_bychunk_geo(config):
    ...
    for i, file_i in enumerate(all_files):
        ...
        for jdx, detected_minimoon in detected_appended_pop_chunk.iterrows():
            ...
            # km and km/s
            asteroid_state = util.helio_eclip_to_geo_secr_generic(asteroid_integrated_states,
                                                                                 asteroid_earth_states, layout="time")
            ...
            # in km, km/s
            spacecraft_state = util.helio_eclip_to_geo_secr_generic(integrated_states, earth_states, layout="time")
            spacecraft_state[:2, :] *= -1

            # convert back to geo eclip based on asteroid epoch
            new_spacecraft_state_geo = util.geo_secr_to_geo_eclip_generic(spacecraft_state,
                                                                                         asteroid_earth_states,
                                                                          layout="time")
            new_asteroid_state_geo = util.geo_secr_to_geo_eclip_generic(asteroid_state,
                                                                                       asteroid_earth_states,
                                                                                       layout="time")

            # convert to geo eme
            new_spacecraft_state_geo_eme = util.geo_eclip_to_geo_eme_generic(new_spacecraft_state_geo, layout="time")
            new_asteroid_state_geo_eme = util.geo_eclip_to_geo_eme_generic(new_asteroid_state_geo, layout="time")
...
def run_sim_runnumbers_MPI_getIOD_data_bychunk_eme(config):
    ...
    for i, file_i in enumerate(all_files):
        ...
        for jdx, detected_minimoon in detected_appended_pop_chunk.iterrows():
            ...
            sc_helio_ini_state = util.geo_secr_to_helio_eclip_generic(sc_secr_ini_state, earth_helio_ini_state)
...
def run_IOD_MPI(config):
    ...
    for file_path in files_for_this_rank:
        ...
        ini_eclip_state = util.geo_eme_to_geo_eclip_generic(ini_eme_state)
        ...
        ini_eme_states = util.helio_eclip_to_geo_eme_generic(ini_helio_states, ini_earth_states, layout="time")
    ...
    for run_idx in range(n_runs):
        ...
        if run_idx % size == rank:
            ...
            if dynamics == '2BD' and orbit == 'GEO' and observer == 'GROUND' and optimizer == 'SGD':
                ...
                from PIELMs import PIELM_basinhopping_w_range_geo_earth as pielm_2ggcb
                parameters = {'NUMBER_OF_OBSERVATIONS': 260, 'OBSERVATION_TIME_FRACTION': 0.5,
                              'TIME_DELTA': 0.0000000001 * u.day,
                              'TOTAL_POINTS': 300, 'SAMPLING_METHOD': "gaussian",
                              'LAYER_RATIOS': [(0., 1 / 10000), (1 / 10000, 9999 / 10000), (9999 / 10000, 1.)],
